...
2. IPNS using DNSLink solves the currency problem but introduces a reliance on internet infrastructure that is ultimately centralized (ICANN and TLD registrars as gatekeepers). While it is possible to support non-ICANN domains using DNS over HTTPS (DoH), IPFS code would need to enshrine specific alternative DNS systems. DNSLink also presents a technical or economic barrier for users as they must have control of or access to a DNS zone in order to register and maintain the mapping to a CID, regardless of whether the domain is acquired through ICANN-sanctioned or alternative registries.
